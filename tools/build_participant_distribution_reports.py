"""Build concentration reports for institutional participant groups."""
...
class ParticipantSpec:
    ...
    market_participation_column: str
    market_volume_column: str
    note: str = ""
...
PARTICIPANTS: dict[str, ParticipantSpec] = {
    "trust": ParticipantSpec(
        ...
        label="\u6295\u4fe1",
        short_label="\u6295\u4fe1",
        participation_column="\u6295\u4fe1\u6210\u4ea4\u53c3\u8207\u7387",
        market_participation_column="trust_participation",
        market_volume_column="trust_volume",
    ),
    "dealer": ParticipantSpec(
        key="dealer",
        label="\u81ea\u71df\u5546",
        short_label="\u81ea\u71df\u5546",
        participation_column="\u81ea\u71df\u5546\u6210\u4ea4\u53c3\u8207\u7387",
        market_participation_column="dealer_participation",
        market_volume_column="dealer_volume",
    ),
    "other": ParticipantSpec(
        key="other",
        label="\u5176\u4ed6\uff08\u6563\u6236\u8fd1\u4f3c\uff09",
        short_label="\u5176\u4ed6",
        participation_column="\u5176\u4ed6\u6210\u4ea4\u53c3\u8207\u7387",
        market_participation_column="other_participation",
        market_volume_column="other_volume",
        note=(
            "\u300c\u5176\u4ed6\u300d\u662f\u4ee5\u5168\u5e02\u6210\u4ea4\u91cf\u6263\u9664\u5916\u8cc7\u3001"
            "\u6295\u4fe1\u3001\u81ea\u71df\u5546\u5f8c\u4f30\u7b97\uff0c\u53ef\u8996\u70ba\u6563\u6236\u8207"
            "\u5176\u4ed6\u975e\u4e09\u5927\u6cd5\u4eba\u4ea4\u6613\u7684\u8fd1\u4f3c\uff0c\u4e0d\u7b49\u540c\u65bc"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build participation concentration reports for trust, dealer, and other groups."
    )
    parser.add_argument(
        "--participants",
        default="trust,dealer,other",
        help="Comma-separated participant keys: trust,dealer,other, or all.",
    )
    parser.add_argument(
        "--date",
        default=None,
...
def load_base_frames(target_date: str | None = None) -> tuple[pd.DataFrame, pd.Series]:
    ...
    return stocks, market_rows.iloc[-1]


def distribution_for(spec: ParticipantSpec, stocks: pd.DataFrame, market_row: pd.Series) -> pd.DataFrame:
    if spec.participation_column not in stocks.columns:
        raise ValueError(f"missing_column:{spec.participation_column}")
    distribution = stocks.copy()
    distribution["Volume"] = pd.to_numeric(distribution["Volume"], errors="coerce")
    distribution["Participation"] = pd.to_numeric(distribution[spec.participation_column], errors="coerce")
    distribution = distribution.dropna(subset=["Volume", "Participation"])
    distribution = distribution[distribution["Volume"].gt(0)]
    distribution["ParticipantVolume"] = distribution["Volume"] * distribution["Participation"]
    ...
    total_volume = float(market_row[spec.market_volume_column])
    if total_volume <= 0:
...
def line_chart_svg(distribution: pd.DataFrame, spec: ParticipantSpec) -> str:
    ...
    return f"""
<svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(spec.label)}\u6210\u4ea4\u91cf\u7d2f\u7a4d\u96c6\u4e2d\u66f2\u7dda">
  <style>
  ...
  {''.join(labels)}
  <text x="{left + chart_width / 2}" y="{height - 2}" text-anchor="middle">\u6309{html.escape(spec.short_label)}\u6210\u4ea4\u91cf\u6392\u540d\u7684\u7d2f\u7a4d\u80a1\u7968\u6bd4\u4f8b</text>
...
"""
    ...
    return f"""
<svg class="chart" viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(spec.label)}\u53c3\u8207\u7387\u5206\u5e03\u76f4\u65b9\u5716">
  <style>
    .axis {{ stroke: #334155; stroke-width: 1; }}
    text {{ fill: #475569; font-size: 12px; font-family: "Microsoft JhengHei", Arial, sans-serif; }}
  </style>
  ...
  <line x1="{left}" y1="{top + chart_height}" x2="{left + chart_width}" y2="{top + chart_height}" class="axis"/>
  {''.join(bars)}
  <text x="{left + chart_width / 2}" y="{height - 5}" text-anchor="middle">\u55ae\u6a94\u80a1\u7968{html.escape(spec.short_label)}\u6210\u4ea4\u53c3\u8207\u7387\u5340\u9593</text>
...
def metric_cards(spec: ParticipantSpec, metrics: dict[str, float | int | str]) -> str:
    card_specs = [
        ...
        (f"\u5e02\u5834{spec.short_label}\u53c3\u8207\u7387", pct(float(metrics["market_participation"]))),
...
def write_report(
    ...
    report_path = DATA_VIZ_ROOT / f"{spec.key}_distribution.html"
    top_80_count = int(metrics["stocks_for_80_share"])
    note_html = f"<p>{html.escape(spec.note)}</p>" if spec.note else ""
    evidence = (
        f"\u6700\u65b0\u4ea4\u6613\u65e5 {html.escape(str(metrics['date']))} "
        f"\u5e02\u5834\u6574\u9ad4{html.escape(spec.short_label)}\u6210\u4ea4\u53c3\u8207\u7387\u70ba {pct(float(metrics['market_participation']))}\uff0c"
        f"\u524d 10 \u6a94\u5df2\u5360 {pct(float(metrics['top_10_share']))}\uff0c"
...
<title>{html.escape(spec.label)}\u53c3\u8207\u96c6\u4e2d\u5ea6\u5831\u544a</title>
<style>
...
<body>
<main>
<h1>{html.escape(spec.label)}\u53c3\u8207\u96c6\u4e2d\u5ea6\u5831\u544a</h1>
...
<section class="panel">
<h2>{html.escape(spec.short_label)}\u6210\u4ea4\u91cf\u96c6\u4e2d\u66f2\u7dda</h2>
<p>\u85cd\u7dda\u8d8a\u9760\u5de6\u4e0a\uff0c\u4ee3\u8868\u8d8a\u5c11\u6578\u80a1\u7968\u8ca2\u737b\u8d8a\u591a{html.escape(spec.short_label)}\u6210\u4ea4\u91cf\uff1b\u7070\u8272\u865b\u7dda\u662f\u5b8c\u5168\u5e73\u5747\u5206\u5e03\u7684\u5c0d\u7167\u3002</p>
{line_chart_svg(distribution, spec)}
</section>
<section class="panel">
<h2>\u55ae\u6a94{html.escape(spec.short_label)}\u6210\u4ea4\u53c3\u8207\u7387\u5206\u5e03</h2>
{histogram_svg(bins, spec)}
</section>
<h2>{html.escape(spec.short_label)}\u6210\u4ea4\u91cf Top 20</h2>
<table>
<thead><tr><th>\u6392\u540d</th><th>\u4ee3\u865f</th><th>\u540d\u7a31</th><th>\u53c3\u8207\u7387</th><th>\u4f30\u7b97\u6210\u4ea4\u91cf</th><th>\u5360\u5168\u5e02{html.escape(spec.short_label)}\u91cf</th><th>\u7d2f\u7a4d\u5360\u6bd4</th></tr></thead>
<tbody>
{table_rows(distribution, 20)}
</tbody>
</table>
<h2>\u7d2f\u7a4d\u9054 80% {html.escape(spec.short_label)}\u6210\u4ea4\u91cf\u80a1\u7968\u6392\u540d</h2>
<p>\u4e0b\u8868\u5217\u51fa\u5f9e\u7b2c 1 \u540d\u7d2f\u7a4d\u5230\u9054\u5168\u5e02{html.escape(spec.short_label)}\u6210\u4ea4\u91cf 80% \u7684\u6240\u6709\u80a1\u7968\uff1b\u672c\u6b21\u5171 {top_80_count:,} \u6a94\u3002</p>
<table>
<thead><tr><th>\u6392\u540d</th><th>\u4ee3\u865f</th><th>\u540d\u7a31</th><th>\u53c3\u8207\u7387</th><th>\u4f30\u7b97\u6210\u4ea4\u91cf</th><th>\u5360\u5168\u5e02{html.escape(spec.short_label)}\u91cf</th><th>\u7d2f\u7a4d\u5360\u6bd4</th></tr></thead>
<tbody>
...
<h2>{html.escape(spec.short_label)}\u504f\u597d\u985e\u578b\u7d71\u6574</h2>
<table>
<thead><tr><th>\u5927\u985e</th><th>\u6a94\u6578</th><th>\u5360\u5168\u5e02{html.escape(spec.short_label)}\u91cf</th><th>\u5360Top80%\u6e05\u55ae{html.escape(spec.short_label)}\u91cf</th><th>\u5e73\u5747\u53c3\u8207\u7387</th><th>\u4ee3\u8868\u80a1</th></tr></thead>
<tbody>
{broad_rows}
</tbody>
</table>
<h2>\u539f\u59cb\u7522\u696d\u7fa4\u7d44\u660e\u7d30</h2>
<table>
<thead><tr><th>\u7522\u696d\u7fa4\u7d44</th><th>\u6a94\u6578</th><th>\u5360\u5168\u5e02{html.escape(spec.short_label)}\u91cf</th><th>\u5360Top80%\u6e05\u55ae{html.escape(spec.short_label)}\u91cf</th><th>\u5e73\u5747\u53c3\u8207\u7387</th><th>\u4ee3\u8868\u80a1</th></tr></thead>
<tbody>
{industry_rows}
</tbody>
</table>
<h2>{html.escape(spec.short_label)}\u53c3\u8207\u7387\u5340\u9593\u6458\u8981</h2>
<table>
...
<p><a href="foreign_distribution.html">\u56de\u5230\u5916\u8cc7\u96c6\u4e2d\u5ea6\u5831\u544a</a> | <a href="index.html">\u56de\u5230\u6cd5\u4eba\u53c3\u8207\u7e3d\u89bd</a></p>
</main>
</body>
</html>
""",
...
def build_report_for(spec: ParticipantSpec, stocks: pd.DataFrame, market_row: pd.Series) -> dict[str, object]:
    distribution = distribution_for(spec, stocks, market_row)
    ...
    metrics: dict[str, float | int | str] = {
        ...
        "total_participant_volume": total_volume,
        "market_participation": float(market_row[spec.market_participation_column]),
    ...
    prefix = f"{spec.key}_distribution"
    top_path = OUTPUT_ROOT / f"{prefix}_top_stocks.csv"
    top_80_path = OUTPUT_ROOT / f"{prefix}_top_80pct_stocks.csv"
    broad_summary_path = OUTPUT_ROOT / f"{prefix}_broad_category_summary.csv"
    industry_summary_path = OUTPUT_ROOT / f"{prefix}_industry_summary.csv"
    bins_path = OUTPUT_ROOT / f"{prefix}_bins.csv"
    metrics_path = OUTPUT_ROOT / f"{prefix}_metrics.csv"
    ...
    broad_summary = summarize_group(top_80_distribution, "BroadCategory", total_volume)
    industry_summary = summarize_group(top_80_distribution, INDUSTRY_GROUP, total_volume)
...
def main() -> None:
    ...
    stocks, market_row = load_base_frames(args.date)
    for spec in specs:
        result = build_report_for(spec, stocks, market_row)
        metrics = result["metrics"]
        print(f"{spec.key} participant distribution report:")
